...
import { GetChart } from "../../util/types";
import { get_chart_data } from "../../services/get_chart_data";
...
const MainChart = () => {
  const { isPending, error, data } = useQuery<GetChart>({
    queryKey: ["ChartsData"],
    queryFn: get_chart_data,
  });
  if (isPending)
    return (
      <div>
  ...
  return (
    <div dir="ltr" className="max-w-full ">
      <ReactECharts className=" !font-iransansnum" option={option} />
    </div>
  );
};
